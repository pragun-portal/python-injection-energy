...
def raw2velocity(filename):	
	#Read from CSV
	value = np.loadtxt(filename, delimiter='\t',skiprows=2, usecols=(2,), unpack=True)
	time = (8e-5)*np.arange(np.size(value))
	#Data Processing
	#Determine Sensor offset
	offset = np.mean(value[1:10])
	print offset
...
def processinjection(time, value):
	...
	peak_mean = value[peaki]# peak_mean = np.mean(value[peaki-10:peaki+10])
...
#pointfile = open("pointfile.txt","w")
...
for filename in filenames:
    if filename.endswith(".csv"):
        ...
        #for i in range(np.size(stream_velocity)):
        #    csvfile.write("%0.5f\t%0.5f\t\n"%(time_stamps[i],stream_velocity[i]))

        #pointfile.write(points_x.__str__())
        #pointfile.write('\n')
        #pointfile.write(points_y.__str__())
        #pointfile.write('\n')
        #pointfile.write(indices.__str__())
        #pointfile.write('\n')
